...
class GUI:
    ...
    def __init__(self):
        ...
        self.window_root.geometry("800x550")
        self.window_root.configure(bg="black")
        ...
        self.frame_data = tkinter.Frame(self.window_root, background = "Blue") # frame for data collection
        self.frame_analytics = tkinter.Frame(self.window_root, background = "Green") # frame for analytics
        self.frame_feedback = tkinter.Frame(self.window_root, background = "Red") # frame for displaying feedback
        ...
        # new button for Baseline
        ttk.Button(self.frame_data, text = "New Baseline", command = self.programmable_function_baseline, width = 20).grid(row = 2, column = 0, sticky = "w", padx = 4, pady = 4)
        ...
        # new button for Sample
        ttk.Button(self.frame_data, text = "New Sample", command = self.programmable_function_sample, width = 20).grid(row = 3, column = 0, sticky = "w", padx = 4, pady = 4)
    ...
    def help_action(self):
        ...
        help_popup.mainloop()
    def programmable_function_baseline(self):
            if self.baseline_dirac is None:
                self.feedback_str.set("Please run a baseline first.")
                return
            baselinedirac = abs(self.baseline_dirac["mean"])
            baselinedirac = str(baselinedirac * 1000) + " mV"
            # sweep_num = 2 # run QC test on the second sweep
            x_baseline0 = [obj[0] * 1000    for obj in self.baseline_fx[0]] # voltage converted to millivolts
            y_baseline0 = [obj[1] * 1000000 for obj in self.baseline_fx[0]] # amps converted to microamps
            x_baseline1 = [obj[0] * 1000    for obj in self.baseline_fx[1]] # voltage converted to millivolts
            y_baseline1 = [obj[1] * 1000000 for obj in self.baseline_fx[1]] # amps converted to microamps
            x_baseline2 = [obj[0] * 1000    for obj in self.baseline_fx[2]] # voltage converted to millivolts
            y_baseline2 = [obj[1] * 1000000 for obj in self.baseline_fx[2]] # amps converted to microamps
            x_baseline3 = [obj[0] * 1000    for obj in self.baseline_fx[3]] # voltage converted to millivolts
            y_baseline3 = [obj[1] * 1000000 for obj in self.baseline_fx[3]] # amps converted to microamps
            x_baseline4 = [obj[0] * 1000    for obj in self.baseline_fx[4]] # voltage converted to millivolts
            y_baseline4 = [obj[1] * 1000000 for obj in self.baseline_fx[4]] # amps converted to microamps
            x_baseline5 = [obj[0] * 1000    for obj in self.baseline_fx[5]] # voltage converted to millivolts
            y_baseline5 = [obj[1] * 1000000 for obj in self.baseline_fx[5]] # amps converted to microamps
            x_baseline6 = [obj[0] * 1000    for obj in self.baseline_fx[6]] # voltage converted to millivolts
            y_baseline6 = [obj[1] * 1000000 for obj in self.baseline_fx[6]] # amps converted to microamps
            x_baseline7 = [obj[0] * 1000    for obj in self.baseline_fx[7]] # voltage converted to millivolts
            y_baseline7 = [obj[1] * 1000000 for obj in self.baseline_fx[7]] # amps converted to microamps
            self.feedback_str.set("Plotting absolute dirac voltage shift to new window")
            self.plot_baselinedirac_voltage(x_baseline0, y_baseline0,x_baseline1, y_baseline1,x_baseline2, y_baseline2, x_baseline3, y_baseline3,x_baseline4, y_baseline4, x_baseline5, y_baseline5, x_baseline6, y_baseline6, x_baseline7, y_baseline7, baselinedirac)
    def programmable_function_sample(self):
            if self.sampling_dirac is None:
                self.feedback_str.set("Please run a sample first.")
                return
            sampledirac = abs(self.sampling_dirac["mean"])
            sampledirac = str(sampledirac * 1000) + " mV"
            sweep_num = 1 # run QC test on the second sweep
            x_sample = [obj[0] * 1000    for obj in self.sampling_fx[sweep_num]] # voltage converted to millivolts
            y_sample = [obj[1] * 1000000 for obj in self.sampling_fx[sweep_num]] # amps converted to microamps
            self.feedback_str.set("Plotting absolute dirac voltage shift to new window")
            self.plot_sampledirac_voltage(x_sample, y_sample, sampledirac)
    def plot_baselinedirac_voltage(self, x_baseline0, y_baseline0,x_baseline1, y_baseline1,x_baseline2, y_baseline2, x_baseline3, y_baseline3,x_baseline4, y_baseline4, x_baseline5, y_baseline5, x_baseline6, y_baseline6, x_baseline7, y_baseline7, delta):
        helpers.print_debug("Plotting dirac voltage graph")
        if self.dirac_plot_fignum is not None and plt.fignum_exists(self.dirac_plot_fignum):
            plt.close(self.dirac_plot_fignum)
        fig, ax = plt.subplots(1, 1)
        ax.plot(x_baseline0, y_baseline0,x_baseline1, y_baseline1,x_baseline2, y_baseline2, x_baseline3, y_baseline3,x_baseline4, y_baseline4, x_baseline5, y_baseline5, x_baseline6, y_baseline6, x_baseline7, y_baseline7, label = "Baseline")
        ax.legend()
        ax.set_xlabel("Voltage (mV)")
        ax.set_ylabel("Current (μA)")
        ax.set_title(f"Baseline voltage (|Δ|={delta})")
        self.dirac_plot_fignum = fig.number
        plt.show()
    def plot_sampledirac_voltage(self, x_sample, y_sample, delta):
        helpers.print_debug("Plotting dirac voltage graph")
        if self.dirac_plot_fignum is not None and plt.fignum_exists(self.dirac_plot_fignum):
            plt.close(self.dirac_plot_fignum)
        fig, ax = plt.subplots(1, 1)
        ax.plot(x_sample, y_sample, label="sample", color="orange")  # Set the color to orange
        ax.legend()
        ax.set_xlabel("Voltage (mV)")
        ax.set_ylabel("Current (μA)")
        ax.set_title(f"sample voltage (|Δ|={delta})")
        self.dirac_plot_fignum = fig.number
        plt.show()